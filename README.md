...
* Step #4: Change the placeholder credentials:
    ...
    1. Edit the ```'.env.local'``` file with the right credentials. NOTE: Make sure you add "us-east-" infront of the "1:" of the REACT_APP_IDENTITY_POOL_ID.
    2. Edit the ```"config"``` variable in the ```'/react-client/src/services/Kendra.ts'``` file with the right credentialsand the Kendra index id (this can found in the CloudFormation console under the “outputs” tab of the stack).
